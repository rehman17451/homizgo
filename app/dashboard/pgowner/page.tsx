...
import {
  ...
  Building, UtensilsCrossed, TrendingUp, Sparkles, BarChart3,
} from "lucide-react"
...
export default function PGOwnerDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!current || current.role !== "pgowner") {
        router.push("/login")
        return
      }
  ...
  const handleAdd = async (data: Partial<Property>) => {
    if (!user) return
    await addProperty({ ...data, ownerId: user.id, ownerName: user.name, ownerRole: "pgowner" } as any)
    ...
    <div className="min-h-screen pb-20 md:pb-0">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Total PGs", value: properties.length, icon: Building, color: "bg-primary/10 text-primary" },
            { label: "Occupancy", value: `${totalOccupants}/${totalCapacity}`, icon: BarChart3, color: "bg-accent/10 text-accent", extra: `${occupancyPercent}%` },
            { label: "Interested", value: properties.reduce((sum, p) => sum + p.interestedUsers.length, 0), icon: Users, color: "bg-chart-4/10 text-chart-4" },
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-baseline gap-2">
                    ...
                    {"extra" in stat && stat.extra && <span className="text-xs font-medium text-muted-foreground">{stat.extra}</span>}
                  </div>
        ...
        {(showForm || editProp) && (
          <div className="mt-8 rounded-2xl border bg-card p-6 shadow-sm">
            <PropertyForm onSubmit={editProp ? handleEdit : handleAdd} onCancel={() => { setShowForm(false); setEditProp(null) }} initial={editProp || undefined} isPG={true} />
          </div>
        )}
            ...
            <div key={prop.id} className="group rounded-2xl border bg-card p-5 transition-all hover:shadow-md hover:border-primary/20">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button variant="outline" size="sm" className="rounded-xl text-destructive hover:bg-destructive/10" onClick={() => handleDelete(prop.id)}><Trash2 className="mr-1.5 h-4 w-4" />Delete</Button>
                </div>
              </div>
                  ...
                  {prop.interestedUsers.length === 0 ? (
                    ...
                    <ul className="mt-2 space-y-1.5">
                      {getInterestedUserNames(prop.interestedUsers).map((name, i) => (
                        <li key={i} className="flex items-center gap-2 text-sm text-card-foreground"><div className="h-2 w-2 rounded-full bg-primary" />{name}</li>
                      ))}
                    </ul>
                  )}
                </div>
